...
export const loader: LoaderFunction = async ({ request, params }) => {
    ...
    if (sessionId && productId) {
        ...
        console.log("isSessionValid form", status, ttl);
        if (status && ttl && kycType && locale) {
            try {
                const productData = await getForm(productId, kycType, locale, sessionId);
